...
def get_pie_chart(entered_site):
    ...
    pie_counts = filtered_df['class'].value_counts().sort_index()
    fig = px.pie(
        values=pie_counts,
...
# ────────────────────────────────────────────────
# Now the callbacks — attach them to actual functions

# Callback for pie chart (only uses site-dropdown)
@app.callback(
...
# Callback for scatter chart (uses BOTH site-dropdown AND payload_slider)
@app.callback(
